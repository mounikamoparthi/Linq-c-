...
namespace ConsoleApplication
{
    public class Program
    {
        public static void Main(string[] args)
        {
            ...
            List<string> WuTang = Artists.Join(Groups, artist => artist.GroupId, group =>group.Id,(artist,group) => { artist.Group = group; return artist;})
                                .Where(artist => artist.Group.GroupName == "Wu-Tang Clan")
                                .Select(artist => artist.ArtistName)
                                .Distinct()
                                .ToList();
            System.Console.WriteLine("All artist names not from Wu-Tang Clan");
